...
//        Cursor cursor = db.rawQuery("select distinct " + COLUMN_C_PROVINCE + "," + COLUMN_C_PINYIN + " from " + TABLE_NAME, null);
        Cursor cursor = db.rawQuery("select a.id,a.c_pinyin,a.c_province from cities a,(select min(c_code) code from cities group by c_province) b where a.c_code=b.code;", null);
